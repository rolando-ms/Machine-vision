...
def thres_adj():
	...
	while(True):
		...
		if(ret):
			...
			'''
			#low = np.array([130, 0, 18])
			#up = np.array([179, 255, 191])
			low = np.array([21, 50, 65])
			up = np.array([36, 255, 122])
			'''
			#low = np.array([0, 42, 0])
			#up = np.array([12, 255, 182])
			#low2 = np.array([155, 42, 0])
			#up2 = np.array([179, 255, 182])
			
			# Threshold the HSV image
			mask = cv2.inRange(hsv, low, up)
			'''
			heigth, width, depth = img.shape
			mask = np.zeros((heigth,width,3), np.uint8)
			mask2 = cv2.inRange(hsv, low, up)
			mask3 = cv2.inRange(hsv, low2, up2)
			mask = cv2.bitwise_or(mask2, mask3, mask)
			'''
